...
- **Browser-Blockaden** – Viele News-Seiten zeigen Consent-Banner oder Anti-Bot-Maßnahmen → Snapshot nach Navigation auswerten, ggf. mehrfach navigieren oder auf alternative seriöse Quellen ausweichen. Bei Bedarf `browser_click` auf Consent-Buttons. Bei leeren oder extrem langen/trunkierten Snapshots (oft >2000 Zeilen mit Navigation/Consent) sofort auf `curl -H "User-Agent: Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36"` mit anschließendem `grep` auf `<h[1-4]>`-Elemente oder Newsticker-Abschnitte umschalten. Dies hat sich in der 27.05.2026-Ausführung als zuverlässigster Fallback erwiesen.
...
- `2026-05-26-cron-outcome.md` (Browser-Fallback mit curl + starkem User-Agent, Wassertemperatur-Quellen-Probleme, Bestätigung des exakten Telegram-Formats bei ruhiger Nachrichtenlage mit Gruppierung und Wassertemperatur-Abschnitt)
