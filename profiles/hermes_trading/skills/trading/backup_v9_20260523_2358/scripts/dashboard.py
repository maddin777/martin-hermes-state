...
def get_cron_jobs():
    ...
    try:
        ...
        for line in output.splitlines():
            ...
            if not any(k in line for k in ["yt_channel","signal_","technical_validator",
...
def get_data():
    ...
    try:
        watchlist = [dict(w) for w in con.execute(
            "SELECT * FROM watchlist WHERE status='watching' ORDER BY conviction_score DESC LIMIT 30"
        ).fetchall()]
    except: watchlist = []
...
def build_html(data):
    ...
    # Source Quality Rows
    source_quality_rows = ""
    for s in data.get("source_quality", []):
        wr  = s.get("win_rate_30d", 0)
        qsc = s.get("quality_score", 0)
        ...
        source_quality_rows += f"""<tr>
            <td><b>{s.get('channel','')}</b></td>
            <td style="text-align:center">{s.get('mentions_30d',0)}</td>
            <td style="text-align:center">{s.get('bought_30d',0)}</td>
            <td style="text-align:center;{wrc}">{wr:.0%}</td>
            <td style="text-align:center">{s.get('avg_pnl_30d',0):+.2f}€</td>
    ...
    for w in data["watchlist"]:
        conv = w.get("conviction_score") or 0
        cc   = "color:#00e676" if conv>=0.6 else "color:#ffd740" if conv>=0.4 else "color:#ff5252"
        tech = f"{w['tech_score']:.2f}" if w.get("tech_score") else "–"
        dc   = "color:#00e676" if w.get("tech_direction")=="LONG" else "color:#ff5252" if w.get("tech_direction")=="SHORT" else "color:#888"
        chs  = json.loads(w.get("channels") or "[]")
        wl_rows += f"""<tr>
            <td>{w['name']}</td><td>{w.get('ticker') or '?'}</td>
            <td>{w.get('sector') or 'Other'}</td>
            <td style="text-align:center">{w.get('mention_count',0)}</td>
            <td style="color:#00e676;text-align:center">{w.get('bullish_count',0)}↑</td>
            <td style="color:#ff5252;text-align:center">{w.get('bearish_count',0)}↓</td>
            <td style="{cc};font-weight:bold;text-align:center">{conv:.0%}</td>
            <td style="text-align:center">{tech}</td>
            <td style="{dc};text-align:center">{w.get('tech_direction') or '–'}</td>
            <td style="font-size:0.8em">{', '.join(list(set(chs))[:3])}</td>
            <td style="font-size:0.8em">{w.get('last_seen','')}</td>
        </tr>"""
    if not wl_rows:
        wl_rows = '<tr><td colspan="11" style="text-align:center;color:#555;padding:20px">Watchlist leer</td></tr>'
...
<div id="tab-watchlist" class="tab-content">
    <h2>📋 Watchlist ({len(data['watchlist'])} Kandidaten, Top 30)</h2>
    <table>
        <tr><th>Unternehmen</th><th>Ticker</th><th>Sektor</th><th>Mentions</th>
        <th>Bull</th><th>Bear</th><th>Conviction</th>
        <th>Tech</th><th>Richtung</th><th>Kanäle</th><th>Zuletzt</th></tr>
        {wl_rows}
    </table>
</div>
        ...
        try:
            ...
            # ── RSS ───────────────────────────────────────────────────────
                ...
                if 0 <= idx < len(sources["rss_feeds"]):
                    ...
                    save_sources(sources)
                    success = True; msg = f"Gewicht aktualisiert"
