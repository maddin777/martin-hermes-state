...
def main():
    ...
    if len(trades) < MIN_TRADES:
        msg = (
            f"📊 <b>Strategy Optimizer</b>\n"
            f"Zu wenig Trades für Optimierung ({len(trades)}/{MIN_TRADES}).\n"
            f"Weiter sammeln und nächste Woche erneut prüfen."
        )
        print(msg)
        send_telegram(msg)
        return
    ...
    if improvement >= IMPROVEMENT_THRESHOLD:
        ...
        msg = (
            f"🔧 <b>Strategie automatisch verbessert!</b>\n\n"
        ...
        msg = (
            f"📊 <b>Wochenbericht Strategy Optimizer</b>\n\n"
            f"Verbesserung zu gering ({improvement*100:+.1f}% < 10%).\n"
            f"Parameter bleiben unverändert.\n\n"
            f"<b>Aktuelle Performance:</b>\n"
            f"• Win Rate:     {current_metrics['win_rate']}%\n"
            f"• Profit Factor:{current_metrics['profit_factor']}\n"
            f"• Sharpe Ratio: {current_metrics['sharpe']}\n"
            f"• Max Drawdown: {current_metrics['max_drawdown']}%\n"
            f"• Trades:       {len(trades)}\n\n"
            f"<b>Beste gefundene Parameter (nicht übernommen):</b>\n"
...
def main():
    ...
    # 3. Original Grid Search (nur bei genug Trades)
    trade_count = con.execute(
        "SELECT COUNT(*) FROM positions WHERE status='closed'"
    ).fetchone()[0]

    if trade_count >= 10:
        print(f"\n🔬 Grid Search ({trade_count} abgeschlossene Trades)...", flush=True)
        _original_main()
    else:
        print(f"\n⏳ Grid Search übersprungen ({trade_count}/10 Trades)", flush=True)
        # Config trotzdem speichern
        with open(CONFIG_PATH, "w") as f:
            json.dump(cfg, f, indent=2)
    ...
    if all_changes:
        changes_text = "\n".join(all_changes)
        msg = (
            f"🔧 <b>Strategy Optimizer - Wochenbericht</b>\n\n"
