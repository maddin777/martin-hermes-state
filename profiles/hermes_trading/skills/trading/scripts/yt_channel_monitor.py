...
def cleanup_db(con):
    # Hoher busy_timeout + Retry, weil diese Methode oft mit DB-Lock crasht
    # wenn fundamental_data/social_scanner noch laufen (Pipeline-Überlapp)
    con.execute("PRAGMA busy_timeout=120000;")
    max_retries = 3
    for attempt in range(max_retries):
        try:
            ...
            if "locked" in str(e) and attempt < max_retries - 1:
                print(f"  ⚠ DB-Lock beim Cleanup (Versuch {attempt+2}/{max_retries}), warte 10s...", flush=True)
                con.rollback()
                time.sleep(10)
            else:
                print(f"  ⚠ Cleanup fehlgeschlagen nach {attempt+1} Versuchen: {e}", flush=True)
                con.rollback()
                return
