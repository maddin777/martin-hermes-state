"""
Factor Ranker — Querschnitts-Ranking ueber das Anlage-Universum.

STAND 19.09.2026 (Option C): Dies ist derzeit ein MOMENTUM-Ranking, kein
Multi-Faktor-Ranking. Quality und Value stehen in thematic_config.json auf
Gewicht 0, weil der Finnhub-Free-Tier weder roicTTM noch freeCashFlowTTM
liefert:

  * value war ueber das gesamte Universum konstant (FCF fehlt durchgaengig).
    Der Code faengt das ab und setzt konstante Faktoren auf neutral 0.5 — er
    kostete also Gewicht, ohne die Rangfolge zu verzerren.
  * quality reduzierte sich auf (1 - debt/equity) und faellt damit bei jedem
    Titel mit debt/equity >= 1 auf exakt 0 — also bei allen Banken
    (JPM 3.30, ING 3.68). Das misst nicht Qualitaet, sondern bestraft einen
    ganzen Sektor.

Zusaetzlich wurde das Universum auf US-Titel beschraenkt: fuer Nicht-US liefert
Finnhub weder Fundamentals noch Recommendations (HTTP 403), wodurch quality und
revision dort strukturell 0 waren — eine eingebaute Bevorzugung von US-Titeln,
die als Faktor-Alpha ausgewiesen worden waere.

Aktive Faktoren: Momentum (0.60), Low Vol (0.25), Revision (0.15).
Quality und Value werden weiterhin BERECHNET und in factor_scores gespeichert
(Diagnose), gehen aber mit Gewicht 0 nicht in den Composite ein.

Rueckweg: Gewichte in thematic_config.json wiederherstellen und universe.json
aus dem Backup zurueckholen, sobald eine Fundamentalquelle mit Nicht-US-
...
# `from thematic.lib import finnhub_client` in den Faktor-Funktionen braucht
# die Projektwurzel auf sys.path; fehlt sie, stirbt der Lauf erst beim ersten
# Ticker. Die Cron-Zeile setzt PYTHONPATH zwar — aber darauf soll sich das
# Modul nicht verlassen muessen.
