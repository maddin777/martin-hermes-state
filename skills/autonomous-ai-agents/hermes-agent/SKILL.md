...
- **Pitfall — Response truncated:** If a cron job repeatedly shows
  `RuntimeError: Response truncated due to output length limit`, the LLM output
  exceeded the agent's response-length limit (checked in `run_agent.py` before
  returning). Common triggers: using `delegate_task` in the prompt (subagent output
  aggregates into the main response), or a model with a low output-token ceiling.
...
- **/goal-Cron-Struktur:** Convert task-getriebene Cron-Prompts in
  ...
  arbeitet über mehrere Turns autonom statt alles in einem Turn zu erledigen.
  - Schreibe den Prompt als: `Setze /goal: <Mission>\n/subgoal 1 — <Teilaufgabe>\n/subgoal N — ...\n/handoff wenn fertig — <finale Vorgaben>`
  - Voraussetzung: `agent.max_turns` (bzw. `goals.max_turns`) ≥ 15
  - **Vorteil:** Jede Quelle/Subtask bekommt eigenen Turn → flexiblere Reihenfolge,
    sauberere Deduplizierung, weniger Context-Druck pro Turn
  - **Beispiel-News-Cron:**
    ```
    ...
    ```
  - **Pitfall:** Der Prompt MUSS mit `Setze /goal:` beginnen, nicht mit "Du sollst".
    Der Agent registriert nur einen expliziten `/goal`-Aufruf als Session-Goal.
    Ein narrativer Prompt wird als einmaliger Task ausgeführt — kein Multi-Turn.
...
### Doctor: system dependency not met

When `hermes doctor` shows `⚠ <toolset> (system dependency not met)`, the
tool's `check_fn` returned `False`. Each toolset has a different check:

- **messaging** — gateway daemon must be running (`hermes gateway status`).
  Profile-based setups (main profile CLI without gateway) see this **harmlessly**.
- **web** — no search/extract API key set (EXA, Parallel, Tavily, Firecrawl).
- **browser-cdp, image_gen, rl, discord** — missing API keys or platform config.
- **computer_use** — macOS-only binary not found.

Full reference with source-code links and per-toolset resolution:
`references/doctor-system-dependencies.md`
...
- **Telegram connect timed out** (`gateway.log`: `telegram connect timed out after 30s`):
  The `TelegramFallbackTransport` (in `gateway/platforms/telegram_network.py`) may use a stale seed IP
  (`149.154.167.220`) when DoH + system DNS discovery fails. The transport wraps `httpx` with IP-rewriting
  TLS, which can time out even when `api.telegram.org` is directly reachable.
  **Pitfall — multi-profile:** If you run isolated profile gateways (`hermes-gateway-hermes-news.service`,
  etc.), each has its own `.env` under `~/.hermes/profiles/<name>/.env`. The `HERMES_TELEGRAM_DISABLE_FALLBACK_IPS=true`
  fix must be added to EVERY profile `.env` that runs its own Telegram gateway. The main profile `.env` at
  `~/.hermes/.env` only controls the main gateway.
  **Pitfall — cron scheduler dependency:** The cron scheduler runs inside the gateway process. If the gateway
  is down (crashed, restart-looped), profile cron jobs (news, lang, trading) will NOT execute. Restoring the
  gateway restores cron delivery. Use a watchdog (see `devops/gateway-watchdog`) for automatic recovery.
  **Diagnosis:**
  1. Test bot token directly: `curl -s "https://api.telegram.org/bot$(grep '^TELEGRAM_BOT_TOKEN=' ~/.hermes/.env | cut -d= -f2)/getMe"`
  2. Test python-telegram-bot directly from the Hermes venv (bypasses the Gateway transport layer)
  3. Check gateway.log for `Telegram fallback-IP transport disabled via env` (confirms fix applied)
  **Fix:** Add to `~/.hermes/.env`:
  ```
  HERMES_TELEGRAM_DISABLE_FALLBACK_IPS=true
  ```
  Then reset service: `systemctl reset-failed hermes-gateway && hermes gateway restart --system`
  Verifies `api.telegram.org` directly. Full diagnostic recipe in `references/gateway-telegram-connect-timeout.md`.
...
Quick reference for configuring optional Hermes features. Requires `/reset` after changes.

### STT (Speech-to-Text) — usually pre-configured

`faster-whisper` is pre-installed in current Hermes releases. Verify:
```bash
python3 -c "import faster_whisper; print(faster_whisper.__version__)"
```
Config is `stt.enabled: true`, `stt.provider: local` in config.yaml. For Groq/OpenAI/Mistral backends, see Voice & Transcription section above.

### Checkpoints (filesystem snapshots) — pre-configured

`checkpoints.enabled: true` is the default. Provides `/rollback N` in-session. Config at `checkpoints.*` in config.yaml: `max_snapshots` (50), `max_total_size_mb` (500), `retention_days` (7).

### Browser Automation

Three backends available: local Chromium (free), Browserbase (cloud), Camofox (cloud). Local setup:
```bash
cd ~/.hermes/hermes-agent && npx playwright install chromium
```
Detailed setup: `references/hermes-browser-automation-setup.md`

### Web Backends (search + extraction)

Hermes supports ddgs (DuckDuckGo), Brave, SearXNG, Firecrawl, Tavily, Exa, Parallel. Free no-API-key option:
```bash
pip install ddgs && hermes config set web.search_backend ddgs
```
Full matrix and setup: `references/hermes-web-backend-setup.md`
