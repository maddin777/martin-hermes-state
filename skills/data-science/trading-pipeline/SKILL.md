...
**⚠️ .py-Cron ohne Wrapper → ModuleNotFoundError (sp500 24.08.):** Der Job `sp500-sma200-check` (1bbecc075d3e) hatte die `script`-Config DIREKT auf `sp500_sma200_check.py` gesetzt. Der no_agent-Harness fiel dann mit dem **Hermes-venv-Python** (KEIN yfinance) in `ModuleNotFoundError: No module named 'yfinance'` — während die Terminal-PYTHONPATH-yfinance vorhanden ist (`/root/.pyenv/versions/3.12.13/bin/python3` hat yfinance 1.3.0). Fix: Wrapper `~/.hermes/scripts/sp500_sma200_check.sh` mit `exec /root/.pyenv/versions/3.12.13/bin/python3 /root/.hermes/scripts/sp500_sma200_check.py` + PYTHONPATH, Cron auf `.sh` umgestellt. Verifiziert: 🟢 AMUMBO HALTEN (+8.15%). **Regel:** no_agent-Trading-`.py` IMMER über `.sh`-Wrapper mit festem pyenv-Python ausführen — nie das Hermes-venv-Python für Markt-Scripts nehmen.
...
| **Pre-Delivery-Verification-Gate (14.09.2026)** → `scripts/pre_delivery_gate.py` validiert die Top-N Watchlist-Kandidaten (Shortlist wie `nightly_eval.calc_top_signals`) VOR der Telegram-Delivery gegen die festen Entry-Kriterien (conviction ≥ min_confidence, tech_score vorhanden, tech_direction LONG, kein `.L`-DQ). Verdrahtet in `nightly_eval.py` (Tages- + Wochen-Report). Zeigt ✅/⚠️/🚫 pro Kandidat als proof-of-work, blockiert aber NIE die Zustellung (graceful Degradation → "❌ nicht verfügbar"). Siehe `references/pre-delivery-gate.md`. |
...
| **UK-Microcap/DQ: `.L`-Ticker verpesten Watchlist (14.08.2026)** → `get_technical_score()` braucht EMA200 (≈200 Bars); HALO.L (92 Bars) → `None` = klassischer DQ-Fall (Status `removed` + DQ-Note, nicht mit Fake-Werten füllen). **ABER: reine Bar-Historie reicht NICHT als Gate** — jeder gescorote Ticker hat eh ≥200 Bars, ein AIM-Nano-Cap mit 200+ Bars bekam trotzdem einen Score. Der eigentliche Differenzierer ist **Liquidität**. Seit 14.08.2026 gilt in `utils.get_technical_score()` ein `.L`-Gate: ≥200 Bars **UND** ≥500k€ 20-Tage-Ø-Turnover, sonst `None` → kein `tech_score`/`tech_direction` → nie ein LONG/SHORT-Entry-Kandidat (Entries brauchen `tech_score>=threshold` + `tech_direction`). **Gotcha (FIXED 16.08.):** `refresh_tech_scores.py` hat früher veraltete Scores NICHT geleert wenn `get_technical_score` `None` liefert (`if tech:`-Guard) → der Eintrag sah entry-fähig aus. **Seit 16.08. cleart der `else`-Zweig** `tech_score/tech_direction/weekly_trend=NULL` (rowcount-gesteuert, meldet "Score gecleart"). Der Guard verhindert zwar neue Scores, aber Bestands-`.L`-Einträge mit NULL tech_score blieben in der Conviction-Watchlist → **DQ-Isolation im Export** (16.08.): `export_watchlist.py` lagert `.L`-Microcaps ohne tech_score in einen separaten `## ⚠️ DQ`-Block aus — sie zählen NICHT in Gesamt/≥76%/Sektor/SHORT-Statistik. **ROOT-CAUSE-FIX (19.08.):** Die DQ-Isolation war ein Symptom-Band-Aid — das Gate blockt zwar Scores, aber die Bestands-`.L`-Einträge AKKUMULIEREN als `watching` (last_seen bleibt frisch → 60d-Stale greift nie). Fix: `watchlist_cleanup.py` Stufe 1b droppt `.L` ohne tech_score (`no-liquidity-gate`), und Cron `7e364ce47b69` läuft jetzt **Mo–Fr 22:30** statt wöchentlich (nach Export, vor DQ-Alarm). Verifiziert: DQ 17→0. **DEAKTIVIERUNGS-VERIFIKATION (24.08.):** Stufe 1c — `.L` MIT tech_score, die zu 100% aus deaktivierten Quellen stammen (`channels`-JSON gegen `source_registry enabled=0`), werden auf `dropped/source-deactivated` gesetzt (z.B. 8 share-talk-Reste). `dq_alarm.py` (`37d505cbc47b`) zählt diese exklusiv-deaktivierten `.L` als eigene Kennzahl `count_deactivated_only()` (bewusst NUR `.L`, sonst False-Positives auf Large-Caps wie STLA/patrick boyle) und alarmiert bei Regression. Siehe `references/uk-microcap-liquidity-gate.md`. |
| **.SG/.MU/.F-Mirror → Pseudo-Shorts + Attributions-Oszillation (10.09.2026)** → `technical_validator.resolve_ticker` bevorzugte DE-Börsen (STU=Stuttgart→`.SG`) → NetApp kam als NTA.SG rein (kein Sektor `Other`, kein tech_score, Short-C>Long-C). Fix: Priorität US>.DE>Sonstige>Nebenboersen + 18 `canonical_tickers`-Regeln + Migration `scripts/canonicalize_watchlist.py` (`--auto`). Parallel: `watchlist.channels` für **bought**-Zeilen war seit Kaufdatum eingefroren (78/82 abweichend) → mischte 2 Zeitbasen → Quellen-Zählungen oszillierten tgl. ±5–15. Fix: channels auch für bought nachziehen, bought ohne Mentions → `[]`. Vollständig: `references/canonical-ticker-mirrors-attribution.md`. |
| **`active_exit_check` öffnet KEINE neuen Trades (verifiziert 16.09.)** → Der Vorschlag "13 neu-tech-SHORT-geflagte bought-Positionen gegen ungewollte Trade-Trigger prüfen" (09:30/15:30-Check) basiert auf einer Metrik-Verwechslung: Der Exit-Check iteriert NUR `positions WHERE status='open'` (nicht die Watchlist), hat KEIN `open_new_positions`/`INSERT INTO positions`, und nutzt live `get_tech_status()` (intact/broken), NICHT die gespeicherte `tech_direction`-Spalte. Die `tech_direction='SHORT'`-Flags liegen auf Watchlist-Einträgen, nicht auf offenen Positionen. **Konsequenz:** Der 09:30/15:30-Check kann per Konstruktion nur Exits/SL-Nachziehen/Trailing liefern — nie einen neuen Entry. Bevor du "Trade-Trigger-Risiko im Exit-Check" vorschlägst: erst prüfen ob der Check überhaupt in die `positions`-Tabelle schreibt (INSERT). |\n| **SHORT-Zählung: `tech_direction` ≠ Sentiment-Short (19.08.)** → Bei Fragen zu "SHORT-Anteil/Welle" IMMER zwei Metriken trennen: (a) `tech_direction='SHORT'` = technischer Indikator sagt Short (viele davon sind **bought** LONG-Positionen, deren Tech-Richtung bearish warnt — kein Bärenregime!), (b) `conviction_score_bear > conviction_score` = sentiment-basiert (echte Bären-Signale, deutlich kleiner). Ein vault-insights-Report der "SHORT 12→19" meldet, zählt oft tech_direction breit → stellt bought LONG-Positionen als Short-Signale dar. Prüf-Query: `COUNT WHERE tech_direction='SHORT'` vs `WHERE conviction_score_bear > conviction_score`. Capture-Bug (POSITIVE→SHORT) bei der Validierung NICHT vorschnell vermuten — die bearish Mentions sind meist echt, das Problem ist die Zählmetrik, nicht der Capture. **Seit 29.08.:** der Watchlist-Export enthält eine `Short-C`-Spalte (=`conviction_score_bear`), vault-insights kann echte Shorts direkt mit `Short-C > Long-C` zählen — keine Bear/Bull-Mentions-Approximation mehr. |
...
| **🔴 search_news braucht `lxml` im Trading-venv (11.09.2026)** → `last30days_gate.py:55` parst Google News RSS mit `BeautifulSoup(resp.text, "xml")` — ohne `lxml` geworfen (bs4 "xml"-Treebuilder fehlt) → `search_news()` returned bei **JEDEM** Ticker nur das `"Fehler: Couldn't find a tree builder..."`-Item (mit der Meldung im Titel, aber trotzdem als einziges Element → `valid=[]`). **Stille Folgen:** (a) `stale_refresh_queue.py --apply` refresht `last_seen` zwar (DB-Update läuft sauber), protokolliert aber `news_count=0/neutral` für 100% der Ticker — News-Sentiment blind; (b) der Pre-Trade-Gate wird durch Fail-Open wirkungslos (immer neutral). **Fix:** `cd <trading> && ./venv/bin/pip install lxml`. Symptom-Check wenn ein Refresh "0 News" meldet: `search_news` manuell aufrufen und prüfen ob das einzige Item `"Fehler:...tree builder"` ist. |
| **Manueller Force-Refresh eines einzelnen Tickers (11.09.2026)** → Bei "Ticker X refresh forcieren" (z.B. weil last_seen stale, `bought`-Position seit Wochen/Monaten ohne Mention): ZWEI Refresh-Aspekte trennen. (1) **Tech-Scores:** `get_technical_score(ticker)` aus `utils` + `UPDATE watchlist SET tech_score=?, tech_direction=?, weekly_trend=?` (oder `refresh_tech_scores.py [--min-conviction]` für die ganze Watchlist). (2) **last_seen + News:** `stale_refresh_queue.py [--top N] [--stale-days 14] [--apply]` (macht Google-News-RSS + frisch last_seen auf heutiges Datum). Wichtig: der DB-`last_seen`-Refresh klappt AUCH wenn News-Parsing fehlschlägt (s.o.) — beide sind unabhängig, verifiziere je Aspekt separat. Verifiziere nachher: `SELECT ticker,last_seen,tech_score,tech_direction FROM watchlist WHERE ticker='X';` UND den `stale_refresh_log`-Eintrag. |
