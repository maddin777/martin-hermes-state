...
## Cost Warning (CHECK BEFORE PROPOSING)

X API is **paid**. Proposing an X-based workflow without mentioning cost first will surprise the user.

| Tier | Cost | Bookmarks | Search | Posts |
|------|------|-----------|--------|-------|
| Free | $0 | no | no | write-only, 1,500/mo |
| Basic | ~$100/mo | 15K/mo | 10K/mo | 50K/mo |
| Pro | ~$5,000/mo | 100K/mo | 1M/mo | 300K/mo |

**Rule: Before proposing any X-API-based workflow (bookmark-monitoring, timeline-scraping, auto-DM, etc.), STATE THE COST FIRST.** Let the user decide before you build anything.

If the user balks at cost, have these **free alternatives** ready:

1. **Telegram-Forward Inbox** — User forwards posts/links to a Telegram chat. Hermes cron picks them up, summarizes, tags, files into Obsidian. Zero cost, works for any URL, not just X. This is the best generic alternative. See `references/telegram-forward-inbox.md` for the full pattern.
2. **Playwright Browser Scrape** — Hermes has Playwright installed. Can log into X headless and scrape bookmarks page. Fragile (UI changes), needs periodic login-cookie refresh.
3. **RSS/Atom Bridges** — Many X accounts have RSS feeds via nitter/rss-bridge. Use the existing `blogwatcher` skill.

---

## Agent Workflow

1. **COST CHECK FIRST.** If the user asks for an X-based monitoring/bookmark/inbox system, state the ~$100/mo Basic tier cost before proceeding. Offer the free alternatives (Telegram-Forward, Playwright, RSS).
2. Verify prerequisites: `xurl --help` and `xurl auth status`.
3. **Check default app has credentials.** Parse the `auth status` output. The default app is marked with an arrow. If the default app shows `oauth2: (none)` but another app has a valid oauth2 user, tell the user to run `xurl auth default <that-app>` to fix it. This is the most common setup mistake.
...
- **Cost:** X API access is **paid**. Basic tier costs ~$100/month (15K bookmarks, 10K searches, 50K posts/month). Free tier is write-only (1,500 posts/mo, no read/bookmark/search). Always state cost before proposing an X-API-based workflow. Have free alternatives ready (Telegram-Forward, Playwright-scrape, RSS — see Cost Warning section above).
